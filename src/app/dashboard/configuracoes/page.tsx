  ...
  const loadConfig = async () => {
    try {
      ...
      if (data && !error) {
        ...
        setConfig({
          ...data,
          nome_dentista: data.nome_dentista ?? 'Dentista Now',
    ...
    <div className="p-3 md:p-6 space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm md:text-base text-purple-300">Gerencie as configurações da dentista</p>
        </div>
        <button
          onClick={handleSave}
          disabled={salvando}
        ...
        <Card className="bg-brand border-brand opacity-90">
          <CardHeader>
            <CardTitle className="text-white flex items-center space-x-2">
              ...
              <span>Informações da Dentista</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="block text-sm text-purple-300 mb-1">Nome da Dentista</label>
              <input
                type="text"
                value={config.nome_dentista}
                onChange={(e) => setConfig({ ...config, nome_dentista: e.target.value })}
