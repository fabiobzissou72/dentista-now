...
curls_to_fix = [
    ...
    ('curl -X POST https://vincedentista.vercel.app/api/clientes/enviar-senha-temporaria', 'curl -X POST https://vincedentista.vercel.app/api/clientes/enviar-senha-temporaria'),
]
...
if '⚠️ IMPORTANTE: AUTENTICAÇÃO' not in content_new:
    content_new = content_new.replace('# 📚 Documentação Completa da API - Vinci Dentista', '# 📚 Documentação Completa da API - Vince Dentista' + warning)
